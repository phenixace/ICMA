            ...
            if target_module_found:
                ...
                if isinstance(target, LoraLayer):
                    ...
                    if loaded_in_8bit and isinstance(target, bnb.nn.Linear8bitLt):
                        ...
                        # infeatures and outfeatures are guided here
...
class GCN(torch.nn.Module):
    def __init__(self, in_features, hidden_features, out_features):
        ...
        self.conv2 = GCNConv(256, hidden_features)
    ...
    def forward(self, data):
        ...
        x = F.relu(x)
        x = F.dropout(x, training=self.training)
        x = self.readout(x.view(-1, 1))
        x = F.dropout(x, training=self.training)

        return x
        
class MolAdapter(nn.Linear, LoraLayer):
    def __init__(
        self,
        adapter_name: str,
        in_features: int,
        ...
        **kwargs,
        ...
        LoraLayer.__init__(self, in_features=in_features, out_features=out_features)
        # create a GNN
        self.gnn = GCN(16, 32, out_features)
    ...
    def forward(self, x: torch.Tensor, graph: torch.Tensor):
        ...
        result += self.beta * self.gnn(graph)   # fuse GNN here
        result = result.to(previous_dtype)

        return result
    
    def merge(self):
        raise RuntimeError("MolAdapter cannot be merged or unmerged")
